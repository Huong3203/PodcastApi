...
// ✅ API MỚI: Check payment status by order_id (cho app gọi sau khi quay lại)
func CheckPaymentStatus(c *gin.Context) {
	orderID := c.Query("order_id")
	if orderID == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Thiếu order_id"})
		return
	}

	payment, err := services.CheckPaymentStatusByOrderID(orderID)
	if err != nil {
