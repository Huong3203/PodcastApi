...
type RegisterInput struct {
	...
	VaiTro  string `json:"vai_tro" binding:"omitempty,oneof=admin user"` // Cho phép chọn vai trò
}
...
func Register(c *gin.Context) {
	...
	// Xác định vai trò: nếu không có thì mặc định là "user"
	vaiTro := input.VaiTro
	if vaiTro == "" {
		vaiTro = "user"
	}
	...
	newUser := models.NguoiDung{
		...
		HoTen:    input.HoTen,
		VaiTro:   vaiTro, // Sử dụng vai trò từ input
		KichHoat: true,
		Provider: "local",
	}
