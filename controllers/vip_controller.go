...
func GetVIPPodcasts(c *gin.Context) {
	...
	// Lọc podcast VIP (sử dụng hàm CheckPodcastVIPStatus từ podcast_controller.go)
	...
	// Gán TomTat (sử dụng hàm AttachSummary từ podcast_controller.go)
...
// ✅ Kiểm tra xem podcast có yêu cầu VIP không (trước khi play) - ADMIN BYPASS
func CheckPodcastVIPRequirement(c *gin.Context) {
	...
	// ✅ Kiểm tra nếu là admin thì bỏ qua VIP check
	role, _ := c.Get("vai_tro")
	if role == "admin" {
		c.JSON(http.StatusOK, gin.H{
			"podcast_id":    podcast.ID,
			"podcast_title": podcast.TieuDe,
			"is_vip":        CheckPodcastVIPStatus(&podcast),
			"can_access":    true,
			"is_admin":      true,
			"message":       "Admin có quyền truy cập mọi podcast",
		})
		return
	}
	...
	// Kiểm tra VIP của user (sử dụng hàm IsUserVIP từ podcast_controller.go)
	var user models.NguoiDung
	if err := db.First(&user, "id = ?", userIDStr).Error; err != nil {
