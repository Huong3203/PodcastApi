...
// UploadAvatarToSupabase uploads user avatar to Supabase Storage
// Path: uploads/avatars/<fileID>.<ext>
func UploadAvatarToSupabase(fileHeader *multipart.FileHeader, fileID string) (string, error) {
	supabaseURL := os.Getenv("SUPABASE_URL")
	supabaseKey := os.Getenv("SUPABASE_KEY")
	...
	ext := filepath.Ext(fileHeader.Filename)
	objectPath := fmt.Sprintf("avatars/%s%s", fileID, ext) // path: uploads/avatars/<fileID>.jpg
